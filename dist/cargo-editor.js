...
export function catalogueOptions(escape,selected='',ammoOnly=false){return `<option value="">Custom / unclassified</option>`+['Ammunition','Equipment','Rations','Repair','Trade Goods'].map(category=>`<optgroup label="${category}">${namedOrder(cargoCatalogue.filter(v=>v.category===category&&(!ammoOnly||category==='Ammunition'))).map(v=>`<option value="${v.id}" ${v.id===selected?'selected':''}>${escape(v.name)}${v.quantityUnit==='cargo unit'?' · per CU':''}</option>`).join('')}</optgroup>`).join('');}
