...
export function createCloud(callbacks){
  ...
  return {
    ...
    setGunWeight:async(id,gunId,value,signature)=>editLoadout(id,s=>{const g=s.guns.find(g=>g.id===gunId);if(!g)throw Error('Gun no longer installed.');g.cargoWeight=weight(value,true);return s;},signature,s=>`${s.guns.find(g=>g.id===gunId)?.name}: cargo weight ${value===null?'default poundage':value+' lb'}`),
    setCapacity:async(id,value,expected)=>{requireAdmin();value=weight(value,true);await runTransaction(db,async tx=>{const ref=shipRef(id),snap=await tx.get(ref);if(!snap.exists())throw Error('Vessel not found.');const old=snap.data();if(old.config.archived)throw Error('Restore this vessel first.');if((old.config.maxLoad??null)!==expected)throw Error('Capacity changed. Review the latest value.');tx.update(ref,{config:{...old.config,maxLoad:value},revision:old.revision+1});activity(tx,crypto.randomUUID(),`${old.config.name}: load capacity ${value===null?'unset':value+' lb'}`);});},
