...
import {refit,moveGun,validateArmament,loadoutSignature} from './weapons.js';
...
export function createCloud(callbacks){
  ...
  return {
    ...
    change:(id,action,text,expected,signature)=>transact([id],([value])=>{if(signature&&signature!==loadoutSignature(unpackShip(value)))throw Error('The loadout changed. Review the vessel and try again.');return [changeShip(value,action,expected)];},text),
    ...
    refit:async(id,slot,type,signature)=>editLoadout(id,s=>refit(s,slot,type),signature,'Updated cannon loadout'),
    moveGun:async(id,from,to,signature)=>editLoadout(id,s=>moveGun(s,from,to),signature,'Moved cannon'),
