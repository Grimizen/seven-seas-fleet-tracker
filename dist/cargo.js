import {compareText,compareNamed,namedOrder} from './ui-order.js';
...
export function filteredInventory(items, filters) {
  ...
  return items.filter(item => (!query || `${item.name} ${item.category}`.toLocaleLowerCase().includes(query))
    ...
    .sort((a,b) => filters.sort==='value-density'?((b.cargoUnits>0?(b.marketValue??0)/b.cargoUnits:-1)-(a.cargoUnits>0?(a.marketValue??0)/a.cargoUnits:-1)||compareNamed(a,b)):filters.sort === 'quantity' ? b.quantity-a.quantity || compareNamed(a,b)
      : filters.sort === 'category' ? compareText(a.category,b.category) || compareNamed(a,b)
      : compareNamed(a,b));
}
export function cargoResults(s,ships,filters,escape) {
  ...
  ${ships.length>1?`<details class="cargo-transfer"><summary>Transfer to another vessel</summary><div class="actions"><label>Transfer amount<input id="transfer-qty-${i.id}" type="number" min="${i.quantityUnit==='cargo unit'?'0':'1'}" step="${i.quantityUnit==='cargo unit'?'any':'1'}" max="${i.quantity}" value="${Math.min(1,i.quantity)}"></label><select id="transfer-to-${i.id}" aria-label="Transfer destination for ${escape(i.name)}">${namedOrder(ships.filter(v=>v.id!==s.id)).map(v=>`<option value="${v.id}">${escape(v.name)}</option>`).join('')}</select><button data-transfer="${i.id}">Transfer</button></div></details>`:''}</article>`).join('')}</div>`}`;
}
export function cargoView(s,ships,filters,escape,canUndo) {
  ...
  <label>Category<select id="cargo-category"><option value="">All categories</option>${[...new Set([...categories,...s.inventory.map(i=>i.category)])].sort(compareText).map(c=>`<option ${filters.category===c?'selected':''}>${escape(c)}</option>`).join('')}</select></label>
