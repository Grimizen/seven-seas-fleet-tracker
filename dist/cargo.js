...
export function cargoResults(s,ships,filters,escape) {
  ...
  `<div class="cargo-list">${items.map(i=>`<article class="cargo-entry"><div class="cargo-name"><h3>${escape(i.name)}</h3><span class="subtle">${escape(i.category)} · ${weightText(i.unitWeight)} each · ${weightText(i.unitWeight==null?null:i.quantity*i.unitWeight)} total</span></div>
  ...
  <details class="cargo-transfer"><summary>Edit item & weight</summary><form class="item-edit-form actions" data-edit-item="${escape(i.id)}"><label>Name<input name="name" required maxlength="80" value="${escape(i.name)}"></label><label>Category<input name="category" required maxlength="40" value="${escape(i.category)}"></label><label>Unit weight (lb)<input name="unitWeight" required type="number" min="0" max="1000000000000" step="any" value="${i.unitWeight??''}"></label><button>Save item</button></form></details>
...
export function cargoView(s,ships,filters,escape,canUndo) {
  ...
  <details class="cargo-add"><summary>Add an item</summary><form id="item-form" class="cargo-add-form"><label>Item name<input id="item-name" required maxlength="80" placeholder="e.g. Repair timber"></label><label>Category<select id="item-category">${categories.map(n=>`<option>${n}</option>`).join('')}</select></label><label>Quantity<input id="item-quantity" required type="number" min="1" step="1" value="1"></label><label>Unit weight (lb)<input id="item-weight" required type="number" min="0" max="1000000000000" step="any" placeholder="Pounds per item"></label><button class="primary">Add item</button></form></details>
