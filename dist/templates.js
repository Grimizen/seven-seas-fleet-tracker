import {weight,capacity} from './load.js';
...
export const templates = ['Small','Medium','Large'].map((size,i)=>({
  ...
  minCrew:[2,3,5][i],maxCrew:[24,36,48][i],maxGuns:[5,7,9][i],capacityUnits:[50,100,150][i],
...
export function reclassify(doc,template){
  ...
  next.config={...old,capacityUnits:capacity(template),armament:a,type:template.type,maxHull:template.maxHull,minCrew:template.minCrew,maxCrew:template.maxCrew,maxGuns:template.maxGuns,stationMax:structuredClone(template.stationMax)};
...
export function createFromTemplate(id,name,template){
  ...
  ship.capacityUnits=capacity(template);
