...
const weighed=()=>{const s=newShip();s.maxLoad=400;s.inventory[0].unitWeight=9;s.guns.forEach(g=>g.loadedUnitWeight=9);return s;};
test('load uses provisional poundage, cargo quantity and separately loaded rounds',()=>{
  const s=weighed(),l=loadState(s);assert.equal(l.known,306);assert.equal(l.remaining,94);assert.deepEqual(l.missing,[]);
  s.guns[0].cargoWeight=100;assert.equal(loadState(s).known,397);
  s.maxLoad=0;assert.equal(loadState(s).over,true);
...
test('legacy unknown weights are incomplete, never silently treated as complete zero load',()=>{
  const l=loadState(newShip());assert.equal(l.known,36);assert.equal(l.capacity,null);assert.equal(l.remaining,null);assert.equal(l.missing.length,5);
...
test('reload and refit conserve weight; firing removes only the loaded round weight',()=>{
  const s=weighed(),fired=apply(s,{type:'fire',id:'gun-1'});assert.equal(loadState(fired).known,297);
  const loaded=apply(fired,{type:'reload',id:'gun-1',ammo:'lead'});assert.equal(loadState(loaded).known,297);
  const removed=refit(loaded,'port-1','');assert.equal(loadState(removed).known,288);
  assert.equal(removed.inventory[0].quantity,26);
  const moved=moveGun(s,'port-1','port-3');assert.equal(loadState(moved).known,306);
});
test('transfer preserves different unit weights as separate rows and conserves combined load',()=>{
  const a=weighed(),b=weighed();b.id='other';b.inventory[0].unitWeight=2;
...
test('cargo weight edits reject stale values; loaded round edits reject a changed round',()=>{
  const d=packShip(weighed()),a={type:'edit-item',id:'lead',name:'Lead shot',category:'Ammunition',unitWeight:8};
  const next=changeShip(d,a,d.game.inventory[0]);assert.equal(next.game.inventory[0].unitWeight,8);
  assert.throws(()=>changeShip(next,a,d.game.inventory[0]),/Another player/);
  const shot={type:'loaded-weight',id:'gun-1',amount:2};
  assert.throws(()=>changeShip(d,shot,{loaded:'Chain shot',weight:9}),/Another player/);
  assert.equal(changeShip(d,shot,{loaded:'Lead shot',weight:9}).game.guns[0].loadedUnitWeight,2);
});
...
test('capacity flows through templates and gun weight changes invalidate stale loadouts',()=>{
  const s=createFromTemplate('x','Test',{...templates[2],maxLoad:123});assert.equal(s.maxLoad,123);
  const old=loadoutSignature(s);s.guns[0].cargoWeight=10;assert.notEqual(loadoutSignature(s),old);
});
